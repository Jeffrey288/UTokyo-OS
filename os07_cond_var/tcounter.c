...
typedef struct {
  ...
  long capacity;
  pthread_mutex_t m[1];
pthread_cond_t c_inc[1];
pthread_cond_t c_dec[1];
} scounter_t;
...
if (pthread_cond_init(s->c_inc, 0)) {
    die("pthread_cond_init, c_inc");
}
if (pthread_cond_init(s->c_dec, 0)) {
    die("pthread_cond_init, c_dec");
}
}
...
while (1) {
    ...
    pthread_cond_wait(s->c_inc, s->m);
}
  x = s->x++;
    pthread_cond_broadcast(s->c_dec);
...
while (1) {
    x = s->x;
    if (x > 0) break;
    pthread_cond_wait(s->c_dec, s->m);
}
  x = s->x--;
    pthread_cond_broadcast(s->c_inc);
  pthread_mutex_unlock(s->m);
    assert(x > 0);
  return x;
}
